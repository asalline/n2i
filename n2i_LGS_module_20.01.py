...
class LGS(nn.Module):
    ...
    def forward(self, f, g_sinograms, operator_split, domain, num_of_splits, device='cuda'):
        ...
        # plt.show()
        u = torch.zeros((g_sinograms.shape[1], num_of_splits-1) + (f.shape[2], f.shape[3])).to(device)
        df = torch.zeros((g_sinograms.shape[1], num_of_splits-1) + (f.shape[2], f.shape[3])).to(device)
