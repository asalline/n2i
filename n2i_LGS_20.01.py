...
def train_network(net, n_train=300, batch_size=25): #g_train, g_test, f_train, f_test, 
    ...
    for i in range(n_train):
        ...
        g_batch = sinogram_split2[n_index,0:num_of_splits-1,:,:].to(device)
        f_batch2 = rec_split[n_index,0:num_of_splits-1,:,:].to(device)
        ...
        outs, step_length2 = net(f_batch2.float(), g_batch.float(), operator_split, domain, num_of_splits)
            ...
            with torch.no_grad():
                ...
                for k in range(test_amount):
                    ...
                    outs4[k,:,:], _ = net(all_arrangements[[k],:,:,:].to(device), all_sinograms[[k],:,:,:].to(device), \
                                       operator_split, domain, num_of_splits+1)
