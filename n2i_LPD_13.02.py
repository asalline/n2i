...
import numpy as np
from n2i_LPD_module import get_images, geometry_and_ray_trafo, LPD, data_split
...
LPD_net = LPD(ray_transform_module, adjoint_operator_module, operator_norm, n_iter=10, device=device).to(device)
...
def train_network(net, n_train=300, batch_size=25, noise=noise): #g_train, g_test, f_train, f_test, 
    ...
    for i in range(n_train):
        ...
        outs = net(f_batch2.float(), g_batch.float(), operator_split, domain, num_of_splits, averaged, noise)
            ...
            with torch.no_grad():
                ...
                for k in range(test_amount):
                    ...
                    # print('hmm2', torch.swapaxes(all_arrangements[[k],:,:,:], 0,1).shape)
                        ...
                        outs4[k,j,:,:] = net(all_arrangements[[k],j,:,:][:,None,:,:].to(device), all_sinograms[[k],j,:,:][:,None,:,:].to(device), \
...
running_loss, running_test_loss, ready_to_eval = train_network(LPD_net, n_train=30001, batch_size=1, noise=noise) # g_train, g_test, f_train, f_test
