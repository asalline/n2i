...
def train_network(net, n_train=300, batch_size=25): #g_train, g_test, f_train, f_test, 
    ...
    for i in range(n_train):
        ...
        f_batch2 = rec_split[n_index,:,:,:].to(device)
        ...
        #     plt.figure()
        #     plt.subplot(1,2,1)
        #     plt.imshow(outs.cpu().detach().numpy())
        #     plt.subplot(1,2,2)
        #     plt.imshow((f_batch[0,0,:,:]-outs).cpu().detach().numpy())
        #     plt.show()
        ...
        if i % 100 == 0:
            ...
            with torch.no_grad():
                ...
                for k in range(test_amount):
                    ...
                    # print('hmm2', all_arrangements[[k],:,:,:].shape)
                    outs4[k,:,:], _ = net(all_arrangements[[k],:,:,:].to(device), all_sinograms[[k],:,:,:].to(device), \
