...
test_input_reco, _, _, _, all_arrangements, all_sinograms, operator_split = data_split(test_amount, num_of_splits, shape, averaged, test_noisy_sinograms, geometry, domain, device, ray_transform)
...
def train_network(net, n_train=300, batch_size=25): #g_train, g_test, f_train, f_test, 
    ...
    for i in range(n_train):
        ...
        if i % train_batch == 0:
            ...
            input_reco, target_reco, sinogram_reco, target_sinogram_reco, _, sinogram_split2, operator_split, \
        ...
        g_batch = sinogram_split2[n_index,:,:,:].to(device)
        f_batch2 = input_reco[n_index,:,:,:].to(device)
        ...
        # print('g_batch', g_batch.device)
        # print('f_batch2', f_batch2.device)
        outs, step_length2 = net(f_batch2.float(), g_batch.float(), operator_split, domain)
        ...
        if i % 100 == 0:
            ...
            with torch.no_grad():
                ...
                outs4 = torch.zeros((test_amount, ) + shape).to(device)
                ...
                # print('hmm', torch.swapaxes(all_arrangements[[k],:,:,:], 0,1).shape)
                for k in range(test_amount):
                    outs3, _ = net(torch.swapaxes(all_arrangements[[k],:,:,:], 0,1).to(device), \
                                   torch.swapaxes(all_sinograms[[k],:,:,:], 0,1).to(device), \
                                       operator_split, domain)
