...
def data_split(num_of_splits, shape, averaged, noisy_sinograms, geometry, domain, device, ray_transform):    
    ...
    rec_split = torch.zeros((2, num_of_splits) + (shape))
    ...
    input_reco = np.zeros((test_amount, ) + shape)
    target_reco = np.zeros((test_amount, ) + shape)
...
input_reco, target_reco, eval_reco, sinogram_split, operator_split = data_split(4, shape, 1, noisy_sinograms, geometry, domain, device, ray_transform)

print(eval_reco.shape)
...
plt.subplot(1,2,2)
plt.imshow(eval_reco[1,0,:,:].cpu().detach().numpy())
plt.show()
