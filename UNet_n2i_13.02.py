...
test_input_reco, _, all_arrangements, _, _ = data_split(test_amount, num_of_splits, shape, averaged, test_noisy_sinograms, geometry, domain, device, ray_transform)
...
def train_network(net, n_train=300, batch_size=25): #g_train, g_test, f_train, f_test, 
    ...
    for i in range(n_train):
        ...
        if i % train_batch == 0:
            ...
            input_reco, target_reco, _, _, _ = data_split(train_batch, num_of_splits, shape, averaged, noisy_sinograms, geometry, domain, device, ray_transform)
        ...
        #     plt.subplot(1,2,2)
        #     plt.imshow((f_batch[0,0,:,:]-outs[0,0,:,:]).cpu().detach().numpy())
        #     plt.show()
        ...
        if i % 100 == 0:
            ...
            with torch.no_grad():
                ...
                for k in range(test_amount):
                    outs3 = net(torch.swapaxes(all_arrangements[[k],:,:,:], 0,1).to(device))
                ...
                # plt.figure()
                # plt.imshow((outs[0,0,:,:] - f_batch[0,0,:,:]).cpu().detach().numpy())
                # plt.show()
            ...
            if i % 1000 == 0:
                ...
                plt.figure()
                plt.subplot(1,2,1)
                plt.imshow(outs4[5,:,:].cpu().detach().numpy())
                plt.subplot(1,2,2)
...
running_loss, running_test_loss, ready_to_eval = train_network(UNet, n_train=30001, batch_size=1) # g_train, g_test, f_train, f_test
