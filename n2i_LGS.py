...
test_input_reco, _, _, _, all_arrangements, _, _ = data_split(test_amount, num_of_splits, shape, averaged, test_noisy_sinograms, geometry, domain, device, ray_transform)
...
LGS_net = LGS(adjoint_operator_module, ray_transform_module, in_channels=2, \
...
def train_network(net, n_train=300, batch_size=25): #g_train, g_test, f_train, f_test, 
    ...
    for i in range(n_train):
        ...
        if i % train_batch == 0:
            ...
            input_reco, target_reco, sinogram_reco, target_sinogram_reco, _, _, _, \
        ...
        g_batch = sinogram_reco[n_index,:,:,:].to(device)
        ...
        print('g_batch', np.shape(g_batch))
        print('f_batch2', np.shape(f_batch2))
        outs = net(f_batch2, g_batch.float())
